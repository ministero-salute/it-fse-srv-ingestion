...
public interface IDocumentCTL extends Serializable {
    ...
    /**
     * Function to process a Creation Request for a document, ingested by the
     * Gateway onto the EDS.
     * The function processes the document by inserting a new record on the staging
     * database, and
     * sending a Kafka message on a topic for the Data Processor Microservice to
     * process.
     * 
     ...
     * @throws EmptyDocumentException         An exception thrown when the document
     *                                        content is empty
     * @throws DocumentAlreadyExistsException An exception thrown when the document
     *                                        already exists on MongoDB
     */
    @PostMapping(value = Constants.Endpoint.PUBLISH_DOCUMENT_PATH, produces = {
    ...
    @ApiResponses(value = {
        ...
        ResponseEntity<DocumentResponseDTO> publishDocument(HttpServletRequest request,
                        @RequestBody SendToUdpDocumentRequestDTO document,
                        @PathVariable("wii") String workflowInstanceId)
                        throws IOException, OperationException, KafkaException, EmptyDocumentException,
                        DocumentAlreadyExistsException;

        /**
         * Function to process a Replace Request for a document, from the Gateway onto
         * the EDS.
         * The function takes in input the new Document DTO and, according to how the
         * flow is set
         * (synchronous or asynchronous, according to the properties file), processes it
         * accordingly
         * 
         * @param request The HTTP Servlet Request
         ...
         * @throws EmptyDocumentException         An exception thrown when the document
         *                                        content is empty
         * @throws UnsupportedOperationException  An exception thrown when the requested
         *                                        operation is not supported
         * @throws DocumentAlreadyExistsException An exception thrown when the document
         *                                        already exists on MongoDB
         * @throws DocumentNotFoundException      An exception thrown when the document
         *                                        is not found on MongoDB
         */
        @PutMapping(value = Constants.Endpoint.REPLACE_DOCUMENT_PATH, produces = {
                        MediaType.APPLICATION_JSON_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE })
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDTO.class))) })
        ResponseEntity<DocumentResponseDTO> replaceDocument(HttpServletRequest request,
                        @RequestBody @Valid SendToUdpDocumentRequestDTO document,
                        @PathVariable String wii) throws OperationException, KafkaException, EmptyDocumentException,
                        UnsupportedOperationException, DocumentAlreadyExistsException, DocumentNotFoundException;

        /**
         * Function to process an Update Request for a document, from the Gateway onto
         * the EDS.
         * The function takes in input the new Document DTO and, according to how the
         * flow is set
         * (synchronous or asynchronous, according to the properties file), processes it
         ...
         *                                        is not found on MongoDB
         */
        @PutMapping(value = Constants.Endpoint.UPDATE_DOCUMENT_PATH, produces = {
                        MediaType.APPLICATION_JSON_VALUE }, consumes = { MediaType.APPLICATION_JSON_VALUE })
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "404", description = "Dictionary non trovato sul database", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = DocumentResponseDTO.class))),
                        @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDTO.class))) })
        ResponseEntity<DocumentResponseDTO> updateDocument(HttpServletRequest request,
                        @RequestBody @Valid SendToUdpDocumentRequestDTO document)
         ...
         */
        @DeleteMapping(value = Constants.Endpoint.DELETE_DOCUMENT_PATH, produces = {
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDTO.class))) })
        ResponseEntity<DocumentResponseDTO> deleteDocument(HttpServletRequest request,
                        @PathVariable @Size(min = DEFAULT_STRING_MIN_SIZE, max = DEFAULT_STRING_MAX_SIZE, message = "identifier does not match the expected size") String identifier)
         ...
         *                                   not found on MongoDB
         */
        @GetMapping(value = Constants.Endpoint.FIND_BY_ID_PATH, produces = {
                        MediaType.APPLICATION_JSON_VALUE })
    ...
    @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDTO.class))) })
        ResponseEntity<UdpDocumentDTO> getDocumentById(HttpServletRequest request,
                        @PathVariable @Size(min = DEFAULT_STRING_MIN_SIZE, max = DEFAULT_STRING_MAX_SIZE, message = "identifier does not match the expected size") String id)
     ...
     */
    @GetMapping(value = Constants.Endpoint.FIND_ALL_PATH, produces = {
            MediaType.APPLICATION_JSON_VALUE })
    ...
    @ApiResponse(content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = UdpDocumentDTO.class)))
    @ApiResponses(value = {
                    @ApiResponse(responseCode = "200", description = "Richiesta Documents avvenuta con successo", content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = UdpDocumentDTO.class))),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ErrorResponseDTO.class))) })
    ResponseEntity<List<UdpDocumentDTO>> getDocuments(HttpServletRequest request);
